 ...
 */
import { NucleobaseCombination, SNP, SNPDB } from '../db/index.js'
 ...
 */
function snpLookup (rsid: string, combination: number): SNPAnalysisResult {
  const lookupResult: SNP|undefined = SNPDB.find(el => el.rsid === rsid)
  const combinationResult: NucleobaseCombination|undefined = lookupResult?.nucleobases.find(el => el.combination === combination)

  const result: SNPAnalysisResult = {
 ...
 */
function nucleobaseTextToBinary (combination: string): number {
  /** Split each acid character */
  const splittedAcids: string[] = combination.split('')

  /** Iterate over each acid, convert it to binary and sum with the next */
  const binarySum = splittedAcids.reduce((sum: number, acid) => {
    const acidValue: number = (AcidBinaryNotationEnum as any)[acid]
...
function nucleobaseBinaryToText (combination: number): string|null {
  ...
  const homozygotes: number[] = [2, 4, 8, 16]
  if (homozygotes.includes(combination)) {
    const acidValue: AcidBinaryNotationEnum = (AcidBinaryNotationEnum as any)[combination / 2]
  ...
  if ((combination & AcidBinaryNotationEnum.T) !== 0) { textRepresentation = textRepresentation.concat('T') }
  if ((combination & AcidBinaryNotationEnum.G) !== 0) { textRepresentation = textRepresentation.concat('G') }
  if ((combination & AcidBinaryNotationEnum.C) !== 0) { textRepresentation = textRepresentation.concat('C') }
  if ((combination & AcidBinaryNotationEnum.A) !== 0) { textRepresentation = textRepresentation.concat('A') }
...
export {
  ...
  snpLookup,
  nucleobaseTextToBinary,
  nucleobaseBinaryToText
}
